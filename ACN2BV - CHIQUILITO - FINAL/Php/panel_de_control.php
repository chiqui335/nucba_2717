...
// Si es paciente, obtener sus turnos
if ($role === 'paciente') {
    ...
    if ($result2->num_rows > 0) {
        ...
        $sql = "SELECT t.id, t.fecha_hora, t.estado, t.nombre_y_apellido_profesional AS profesional, t.especialidad
                FROM turno t
                WHERE t.id_paciente = ?";
...
elseif (in_array($role, ['medico', 'psicologo', 'kinesiologo'])) {
    ...
    if ($result2->num_rows > 0) {
        ...
        $sql = "SELECT t.id, t.fecha_hora, t.estado, 
                       ...
                       p.os AS obra_social, p.nro_credencial
                FROM turno t
